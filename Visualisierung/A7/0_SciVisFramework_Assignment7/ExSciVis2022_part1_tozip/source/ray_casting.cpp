    ...
    )
{
    ...
    index = (int)f.find("#define ENABLE_LIGHTING");
    f.replace(index + 24, 1, ss3.str());


    std::stringstream ss4;
    ss4 << binary_search_on;

    index = (int)f.find("#define ENABLE_BINARY_SEARCH");
    f.replace(index + 29, 1, ss4.str());
...
void showGUI(){
    ...
    {        
        ...
        ImGui::RadioButton("Maximum Intensity Projection", &g_task_chosen, 12);
        ImGui::RadioButton("First-hit Iso-Surface Ray_casting", &g_task_chosen, 13);
    ...
    while (!g_win.shouldClose()) {
        ...
        if (g_reload_shader){
            ...
            try {
                ...
                newProgram = loadShaders(g_file_vertex_shader, g_file_fragment_shader, g_task_chosen, g_lighting_toggle, g_binary_search_toggle);
                g_error_message = "";
            }
            catch (std::logic_error& e) {
                //std::cerr << e.what() << std::endl;
